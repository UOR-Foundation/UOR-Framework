...
mod tests {
    ...
    fn index_has_all_terms() {
        ...
        assert_eq!(index.classes.len(), 200);
        assert_eq!(index.properties.len(), 386);
        assert_eq!(index.individuals.len(), 712);
    }
    ...
    fn directive_expansion_count() {
        ...
        let expanded = renderer::expand_directives(src, &index);
        assert!(
            expanded.contains("200"),
            "Count directive should expand to 200, got: {}",
            expanded
        );
    }
}
