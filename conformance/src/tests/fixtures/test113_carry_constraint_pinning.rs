/// SHACL test 113: Carry constraint fiber-pinning map identities —
/// demonstrates CC_PINS and CC_COST_FIBER identity grounding (Amendment 44).
...
op:CC_PINS a owl:NamedIndividual, op:Identity ;
    rdfs:label "CC_PINS" ;
    op:lhs "pinsFibers(CarryConstraint(p))" ;
...
op:CC_COST_FIBER a owl:NamedIndividual, op:Identity ;
    rdfs:label "CC_COST_FIBER" ;
    op:lhs "|pinsFibers(CarryConstraint(p))|" ;
    op:rhs "popcount(p) + 1" ;
    op:forAll "bit-pattern p in CarryConstraint" ;
...
proof:prf_CC_COST_FIBER a owl:NamedIndividual, proof:ComputationCertificate ;
    proof:provesIdentity op:CC_COST_FIBER ;
    proof:atQuantumLevel <https://uor.foundation/schema/Q0> .
"#;
