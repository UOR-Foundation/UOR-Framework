...
//! crate's own tests.
//!
//! **Operand-support caveat.** The macros accept operand types whose
//! `CONSTRAINTS` arrays contain only `Residue`, `Hamming`, `Depth`, `Carry`,
//! `Site`, `SatClauses`, and `Bound` variants. Operands containing `Affine`
//! or `Conjunction` constraints are not supported because stable Rust const
//! evaluation cannot rebuild a shifted `&'static [i64]` coefficient slice at
//! macro-expansion time. The underlying `shift_constraint` helper in
//! foundation's `pipeline.rs` returns a `Site { position: u32::MAX }`
//! sentinel for those variants, which the combined shape's
//! `validate_const()` catches at use-time as a typed failure rather than
//! silently-wrong output. Consumers needing `Affine`-bearing operands use
//! the manual-construction pattern from the amendment's §Gap 2.
