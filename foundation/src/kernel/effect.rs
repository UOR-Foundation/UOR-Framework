...
//! `effect/` namespace — Typed endomorphisms on state:Context classified by fiber target. Formalizes what cascade guard-effect pairs do to the fiber budget..
//!
//! Space: Kernel

use crate::Primitives;

/// A typed endomorphism on state:Context. Maps one fiber-budget configuration to another. The atomic unit of state mutation in the kernel.
pub trait Effect<P: Primitives> {
    /// Associated type for `EffectTarget`.
    type EffectTarget: EffectTarget<P>;
    /// The fiber coordinates this effect touches.
    ...
    /// Change in freeCount: −1 for PinningEffect, +1 for UnbindingEffect, 0 for PhaseEffect.
    fn free_count_delta(&self) -> P::Integer;
...
/// Pins a single free fiber to a definite value. Decrements freeCount by exactly 1. The effect produced by constraint resolution.
pub trait PinningEffect<P: Primitives>: ReversibleEffect<P> {}

/// Releases a pinned fiber back to free state. Increments freeCount by exactly 1. The effect produced by session boundary reset.
...
/// An opaque effect declared by a Prism implementation. The kernel treats it as a fiber-budget transformation satisfying the declared commutation contract. Must carry an effect:externalEffectShape linking to a conformance:EffectShape.
...
/// The set of fiber coordinates that an effect reads or writes. Determines commutation.
pub trait EffectTarget<P: Primitives> {
    ...
    /// The individual fiber coordinates in this target set.
    fn target_fibers(&self) -> &[Self::FiberCoordinate];
    /// Number of fiber coordinates in this target set.
